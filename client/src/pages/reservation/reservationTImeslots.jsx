...
import { fetchReservationId,  deleteReservation,fetchReservationByItemId,updateReservationData  } from '../../store/actions/ReservationAction';
import {  fetchReservationItemsById, fetchTimeSlotsByItemId, } from '../../store/actions/ReservationItemActions';
...
const ReservationGroupList = () => {
  ...
  const calculateAvailableReservations = () => {
    ...
    const reservationsWithinTimeRange = reservationByItem.filter(reservation => {
      return (reservation.time1 <= time2 && reservation.time2 >= time1) ||
    ...
    const availableReservation = fetchItem.noOfReservations - totalreservationsWithinTimeRange;
  ...
  const calculateAvailableCapacity = () => {
    if (slotType !== "Flexible") {
      ...
      const reservationsWithinTimeRangeFle = reservationByItem.filter(reservation => {
        return (reservation.time1 <= time2 && reservation.time2 >= time1) ||
        (reservation.time1 <= time1 && reservation.time2 >= time2);
      });
      ...
      const totalPeopleWithinRange = reservationsWithinRange.reduce((total, reservation) => {
        return total + reservation.noOfPeople;
      }, 0);
  ...
  const handleDiscard = () => {
    if (reservationData.id) { 
      ...
      dispatch(deleteReservation(reservationData.id));
      navigate(`/reservation/createReservation`);
    }
  };
      ...
      <Col xs={0} sm={0} md={2} lg={2} xl={2} xxl={1} />
        ...
        >
          <div>
            ...
            <Col xs={12} md={12}>
            {slotType==="Defined" &&
             <>      <TextField id="date" type="date" label="Date:"onChange={handleInputChange} />
             {dateSelected===true &&
      ...
      <option key={item.id} value={`${item.startTime}-${item.endTime}`}>
        {item.startTime} - {item.endTime}
              ...
              <Row>
                ...
                <Col xs={12} md={6}>
                  <TextField id="time1" type="datetime-local" label="From:"onChange={handleInputChange} />
                </Col>

                <Col xs={12} md={6}>
                  <TextField id="time2" type="datetime-local" label="To:"onChange={handleInputChange} />
                </Col>
              </Row>
}
...
{slotType === "Flexible"&&
  ...
  />
              ...
              <TextField id="noOfPeople" label="No of People :" type="text" onChange={handleInputChange}  />
              {showMessage &&
             <><span id="message">More than available Capacity</span><br></br></>
              }
